...
const messageReducer = (state, action) => {
  ...
  const { email, message, messages, reaction } = action.payload;
  ...
  switch(action.type) {
    ...
    case 'ADD_REACTION':
      ...
      const messageIndex = userCopy.messages?.findIndex(m => m._id === reaction.message._id);
      if(messageIndex > -1) {
        // Make a shallow copy of user messages
        let messagesCopy = [...userCopy.messages];
        // Make a shallow copy of message reactions
